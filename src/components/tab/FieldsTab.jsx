...
export default function FieldsTab() {
    ...
    const filtered = useMemo(() => {
        ...
        return items.filter((it) => it.name?.toLowerCase().includes(s) || it.field_type?.toLowerCase().includes(s));
    }, [items, search]);

    const load = async () => {
        setLoading(true);
        try {
            const [fieldRes, deviceRes] = await Promise.all([
                getDataApi("/fields", { limit: 100, page: 1 }),
                getDataApi("/devices", { limit: 100, page: 1 })
            ]);
            setItems(fieldRes.data?.items || fieldRes.data || []);
            setDevices(deviceRes.data?.items || deviceRes.data || []);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="space-y-6">
                ...
                <form id="field-form" onSubmit={onSubmit} className="grid grid-cols-12 gap-4">
                    ...
                    <div className="col-span-12 lg:col-span-7 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Tên nơi trồng</label>
                                <input
                                    className={inputCls}
                                    value={form.name}
            ...
            <div className="flex items-center justify-between">
                <div className="text-sm text-gray-500 text-nowrap mr-4">
                    {loading ? "Đang tải..." : `Tổng: ${items.length} nơi trồng`}
                </div>
                <input
