...
export default function ProductsTab() {
    ...
    const filtered = useMemo(() => {
        ...
        return items.filter((it) => it.name?.toLowerCase().includes(s) || it.type?.toLowerCase().includes(s));
    }, [items, search]);

    const load = async () => {
        setLoading(true);
        try {
            const [productRes, fieldRes] = await Promise.all([
                getDataApi("/products", { limit: 100, page: 1 }),
                getDataApi("/fields", { limit: 100, page: 1 })
            ]);
            setItems(productRes.data?.items || productRes.data || []);
            setFields(fieldRes.data?.items || fieldRes.data || []);
        } finally {
            setLoading(false);
        }
    };

    // chỉ reload danh sách field
    const reloadFields = async () => {
        try {
            const res = await getDataApi("/fields", { limit: 100, page: 1 });
            setFields(res.data?.items || res.data || []);
        } catch (e) {
            console.error("Reload fields failed", e);
        }
    };
    ...
    // Nghe sự kiện từ tab Nơi trồng để reload dropdown Nơi trồng
    useEffect(() => {
        const onFieldsChanged = () => reloadFields();
        const onStorage = (e) => {
            if (e.key === "fields_changed_at") reloadFields(); // nếu bạn setItem ở FieldsTab
        };
        window.addEventListener("fields:changed", onFieldsChanged);
        window.addEventListener("storage", onStorage);
        return () => {
            window.removeEventListener("fields:changed", onFieldsChanged);
    ...
    const onSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await patchDataApi(`/products/${editingId}`, payload);
            } else {
                await postDataApi("/products", payload);
            }
            setForm(initialForm);
            setEditingId(null);
            await load();
    ...
    const onDelete = async (id) => {
        ...
        await deleteDataApi(`/products/${id}`);
        await load();
    };
        ...
        <div className="space-y-6">
                ...
                <form id="product-form" onSubmit={onSubmit} className="grid grid-cols-12 gap-4">
                    ...
                    <div className="col-span-12 lg:col-span-6 space-y-4">
                        <div>
                            <div className="flex items-center justify-between mb-1">
                                ...
                                <button
                                    ...
                                    title="Tải lại danh sách nơi trồng"
                                >
                                    Tải lại
                                </button>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Gieo trồng</label>
                                <input
                                    type="date"
                                    className={inputCls}
                    ...
                    <div className="col-span-12 lg:col-span-6 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Đơn vị cân nặng</label>
                                <input
                                    className={inputCls}
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 rounded-lg border border-gray-200/80 p-1 bg-gray-50 w-full">
                                {[
                                    { v: "growing", label: "Đang trồng" },
                                    { v: "harvesting", label: "Thu hoạch" },
                                    { v: "procesing", label: "Sơ chế" }
                                ].map((o) => (
                                    <button
                                        key={o.v}
                                        type="button"
                                        data-active={form.status === o.v}
                                        onClick={() => setForm({ ...form, status: o.v })}
                                        className={segBtn + " mr-1 last:mr-0 min-w-24"}
                                    >
                                        {o.label}
                                    </button>
                                ))}
            ...
            <div className="flex items-center justify-between">
                <div className="text-sm text-gray-500 text-nowrap mr-4">
                    {loading ? "Đang tải..." : `Tổng: ${items.length} nông sản`}
                </div>
                <input
                    ...
                    <table className="min-w-full text-sm">
                        <thead className="bg-gray-50 text-gray-700 sticky top-0">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left font-semibold">Nơi trồng</th>
                                <th className="px-4 py-3 text-left font-semibold">Trạng thái</th>
                                <th className="px-4 py-3 text-left font-semibold">Giá/đv</th>
